...
tags: [ComputerGraphics, Rendering, UnrealEngine, DistanceField, RayTracing, Lumen]
...
각 클립맵은 다시 **페이지(page)** 단위로 쪼개진다(페이지 = `7³` 복셀, 아틀라스에서는 경계 포함 `8³`). 그리고 표면 근처 페이지만 실제로 할당된다. 텅 빈 하늘 같은 영역엔 페이지를 안 만든다. 이건 VT/VSM에서 본 “필요한 것만 적재한다”는 같은 방식이고, 클립맵 UV→물리 페이지를 잇는 페이지 테이블(`PF_R32_UINT`) 인다이렉션도 동일한 구조다.
...
그래서 “메시 디스턴스 필드가 글로벌 디스턴스 필드 아니냐”는 질문의 답은 **재료는 같지만 층위가 다르다**. 로컬 SDF들을 “가장 가까운 거리만 고르기”로 접어 씬 하나짜리 distance field로 만들고, 그 위를 스피어 트레이싱으로 행진하는 순간 Unreal은 삼각형 없이도 월드 전체를 향해 레이를 쏠 수 있게 된다. Lumen·DFAO·MegaLights는 모두 이 한 장을 바탕으로 움직인다.
