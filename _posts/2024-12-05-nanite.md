...
icon: paper
permalink: nanite-architecture-cost-analysis
...
> - MeshDrawCommand 캐싱에서 MaterialPass까지의 전체 흐름
...
<div class="callout callout-info" style="margin-bottom:40px;">
  ...
  <p>이제 위 세 아이디어를 실제 코드 수준으로 파헤친다. Cluster 계층 구조 → MeshDrawCommand 캐싱 → GPU/CPU 패스 흐름 → 비용 분석 → 각 패스의 상세 동작 순서로 진행된다. 앞의 직관을 떠올리며 읽으면 각 단계가 왜 존재하는지 자연스럽게 연결된다.</p>
</div>
...
<span class="section-eyebrow">00 — MeshDrawCommand 캐싱</span>
...
<p style="color:var(--text2);line-height:1.85;">
  Nanite는 씬 프록시가 FScene에 등록될 때 <strong>MeshDrawCommand를 미리 캐싱</strong>한다. 이 캐싱 결과물이 런타임 내내 래스터라이즈·셰이딩 단계에서 재사용된다. 캐싱 과정에서 두 가지 핵심 GPU-side ID가 생성된다.
</p>

<div class="two-col">
  <div class="col-box">
    <h4>RasterBin — 래스터라이즈용 머티리얼 ID</h4>
    <ul>
      <li><span class="li-name">용도</span><span class="li-val" style="color:var(--text2);font-family:inherit;font-size:12px;">Visibility Buffer 생성 단계</span></li>
      <li><span class="li-name">일반 머티리얼</span><span class="li-val" style="color:var(--text2);font-family:inherit;font-size:12px;">Default 머티리얼 사용</span></li>
      <li><span class="li-name">Programmable</span><span class="li-val" style="color:var(--text2);font-family:inherit;font-size:12px;">실제 머티리얼 사용</span></li>
      <li><span class="li-name">Programmable 조건</span><span class="li-val" style="color:var(--text2);font-family:inherit;font-size:12px;">WPO·PDO·Masked·Displacement</span></li>
    </ul>
    <p style="font-size:12px;color:var(--text2);margin-top:12px;line-height:1.65;">
      WPO처럼 버텍스 위치가 바뀌는 경우 Default 머티리얼로 래스터라이즈하면 실루엣이 틀어진다. 이런 <em>Programmable</em> 머티리얼만 실제 머티리얼로 래스터라이즈하고, 나머지는 저렴한 Default 머티리얼을 사용한다.
    </p>
  </div>
  <div class="col-box">
    <h4>ShadingBin — 셰이딩용 머티리얼 ID</h4>
    <ul>
      <li><span class="li-name">용도</span><span class="li-val" style="color:var(--text2);font-family:inherit;font-size:12px;">GBuffer 생성 단계</span></li>
      <li><span class="li-name">항상</span><span class="li-val" style="color:var(--text2);font-family:inherit;font-size:12px;">실제 머티리얼 사용</span></li>
      <li><span class="li-name">할당 방식</span><span class="li-val" style="color:var(--text2);font-family:inherit;font-size:12px;">MaterialSlotAllocator</span></li>
      <li><span class="li-name">GPU 접근</span><span class="li-val" style="color:var(--text2);font-family:inherit;font-size:12px;">LoadMaterialSlot(PrimIdx, MatIdx)</span></li>
    </ul>
    <p style="font-size:12px;color:var(--text2);margin-top:12px;line-height:1.65;">
      ShadingBin은 ShadingMask에 기록되어 EmitDepthTargets에서 읽히고, MaterialDepth 값으로 변환되어 BasePass DepthEqual 판별에 사용된다.
    </p>
  </div>
</div>

<div class="callout callout-info">
  <div class="callout-title">💡 MaterialSlot — CPU↔GPU 브릿지</div>
  <p><code>FNaniteMaterialSlot</code>은 RasterBin(래스터 단계)과 ShadingBin(셰이딩 단계) 정보를 하나로 묶은 구조체다. 프리미티브 등록 시 CPU에서 생성하고, GPUScene 업데이트 시점에 GPU로 업로드된다. GPU 셰이더는 <code>PrimitiveIndex × MaxMaterials(64) + MaterialIndex</code> 오프셋으로 해당 슬롯을 읽어 현재 클러스터의 머티리얼 정보를 얻는다.</p>
  <p>MeshDrawCommand 자체는 런타임에 재생성되지 않고 캐싱되어 재사용된다. 씬 변경(오브젝트 추가·제거·머티리얼 변경) 시에만 캐시가 무효화되고 재빌딩되며, 이것이 CPU 프로파일의 <code>FPrimitiveSceneInfo_CacheNaniteMaterialBins</code> 비용(Incl ~2%)으로 나타난다.</p>
</div>

<span class="section-eyebrow">01 — 패스 흐름</span>
...
<div class="callout callout-info">
  ...
  <p>CPU의 C3~C6는 실제 렌더링이 아니라 <strong>RDG 커맨드 레코딩</strong>이다. 실제 GPU 실행은 RDG ExecutePass 시점에 이루어진다. CPU 프로파일링의 BasePass 비용(Excl ~15%)이 높은 것은 머티리얼 수만큼 드로콜을 빌딩하는 C6 작업 때문이다.</p>
</div>

<span class="section-eyebrow">02 — GPU 비용</span>
...
<p style="color:var(--text2);line-height:1.85;">
  <strong>Excl</strong> 값이 해당 패스 자체가 소비한 순수 GPU 시간, <strong>Incl</strong>는 자식 패스를 포함한 총 시간이다. 병목 분석은 Excl에 집중한다.
</p>

<table class="mapping-table">
  <thead>
    <tr><th>Pass</th><th style="text-align:right;">Count %</th><th style="text-align:right;">Incl %</th><th style="text-align:right;">Excl %</th><th>설명</th></tr>
  </thead>
  <tbody>
    <tr><td class="mono-cell">Nanite::DrawGeometry</td><td class="num-cell">100%</td><td class="num-cell">100%</td><td class="num-cell">~4%</td><td class="desc-cell">컬링+래스터 전체. Excl=컬링 디스패치 오버헤드</td></tr>
    <tr><td class="mono-cell">Nanite::VisBuffer</td><td class="num-cell">~67%</td><td class="num-cell">~54%</td><td class="num-cell">~0.1%</td><td class="desc-cell">래스터 페이즈 전체. 실제 비용은 자식 패스</td></tr>
    <tr><td class="mono-cell">NaniteBasePass</td><td class="num-cell">~67%</td><td class="num-cell">~29%</td><td class="num-cell">~0.2%</td><td class="desc-cell">머티리얼 셰이딩 페이즈 컨테이너</td></tr>
    <tr class="highlight-row"><td class="mono-cell">Nanite::BasePass</td><td class="num-cell">~33%</td><td class="num-cell">~23%</td><td class="num-cell excl-cell">~19%</td><td class="desc-cell"><strong>GPU 단일 패스 최대 비용.</strong> 머티리얼별 Indirect DrawCall → GBuffer</td></tr>
    <tr class="highlight-row"><td class="mono-cell">Nanite::Readback</td><td class="num-cell">~33%</td><td class="num-cell">~8%</td><td class="num-cell excl-cell">~8%</td><td class="desc-cell">GPU→CPU 스트리밍 피드백. 동기 읽기</td></tr>
    <tr class="highlight-row"><td class="mono-cell">Nanite::EmitDepthTargets</td><td class="num-cell">~33%</td><td class="num-cell">~5%</td><td class="num-cell excl-cell">~5%</td><td class="desc-cell">VisBuffer → Depth·Stencil·MaterialDepth (FullScreen ×3)</td></tr>
    <tr class="highlight-row"><td class="mono-cell">Nanite::ShadeBinning</td><td class="num-cell">~33%</td><td class="num-cell">~4%</td><td class="num-cell excl-cell">~4%</td><td class="desc-cell">머티리얼 타일 분류 3 Compute 패스</td></tr>
    <tr><td class="mono-cell">Nanite::RasterizeLumenCards</td><td class="num-cell">~33%</td><td class="num-cell">~7%</td><td class="num-cell">~0%</td><td class="desc-cell">Lumen 캡처용 래스터라이즈</td></tr>
    <tr><td class="mono-cell">Nanite::InitContext</td><td class="num-cell">~67%</td><td class="num-cell">~2%</td><td class="num-cell">~2%</td><td class="desc-cell">View·Buffer 초기화</td></tr>
  </tbody>
</table>
<p style="color:var(--text3);font-size:12px;margin-top:-16px;">* 기준: Nanite::DrawGeometry Incl = 100%. Count·Incl·Excl 모두 이 값 대비 비율.</p>

<div class="callout callout-warn">
  <div class="callout-title">⚡ GPU 병목 핵심</div>
  <p>DrawGeometry Incl(100%)은 자식 패스를 포함한 계층 합산이다. 순수 컬링 오버헤드(Excl ~4%)는 전체 대비 작다. <strong>실질 GPU 최대 병목은 BasePass 머티리얼 셰이딩(Excl ~19%)</strong>이며, 씬 내 유니크 Nanite 머티리얼 수에 비례한다.</p>
</div>

<span class="section-eyebrow">03 — CPU 비용</span>
...
<table class="mapping-table">
  ...
  <tbody>
    <tr><td class="mono-cell">Nanite::DrawGeometry</td><td class="num-cell">100%</td><td class="num-cell">100%</td><td class="num-cell">~3%</td><td class="desc-cell">컬링 전체 RDG 레코딩 루트</td></tr>
    <tr><td class="mono-cell">Nanite::VisBuffer</td><td class="num-cell">~49%</td><td class="num-cell">~52%</td><td class="num-cell">~0.2%</td><td class="desc-cell">VisBuffer 페이즈 루트</td></tr>
    <tr><td class="mono-cell">NaniteBasePass</td><td class="num-cell">~24%</td><td class="num-cell">~19%</td><td class="num-cell">~0.4%</td><td class="desc-cell">머티리얼 셰이딩 페이즈 루트</td></tr>
    <tr class="highlight-row"><td class="mono-cell">Nanite::BasePass</td><td class="num-cell">~17%</td><td class="num-cell">~17%</td><td class="num-cell excl-cell">~15%</td><td class="desc-cell"><strong>CPU 최대 비용.</strong> FNaniteMaterialPassCommand 빌딩 + Indirect DrawCall 등록</td></tr>
    <tr><td class="mono-cell">InitNaniteRaster</td><td class="num-cell">~2%</td><td class="num-cell">~5%</td><td class="num-cell">~0.1%</td><td class="desc-cell">FRasterizerPass 생성 (캐싱됨)</td></tr>
    <tr><td class="mono-cell">Nanite::RasterizeLumenCards</td><td class="num-cell">~11%</td><td class="num-cell">~5%</td><td class="num-cell">~0.1%</td><td class="desc-cell">Lumen 래스터 레코딩</td></tr>
    <tr><td class="mono-cell">FVirtualShadowMapArray::<wbr>RenderVSMNanite</td><td class="num-cell">~2%</td><td class="num-cell">~5%</td><td class="num-cell">~0%</td><td class="desc-cell">VSM Nanite 렌더링 레코딩</td></tr>
    <tr><td class="mono-cell">Nanite::LumenMeshCapturePass</td><td class="num-cell">~7%</td><td class="num-cell">~3%</td><td class="num-cell">~0.6%</td><td class="desc-cell">Lumen 카드 캡처 레코딩</td></tr>
    <tr class="highlight-row"><td class="mono-cell">FPrimitiveSceneInfo_<wbr>CacheNaniteMaterialBins</td><td class="num-cell">~0.3%</td><td class="num-cell">~2%</td><td class="num-cell excl-cell">~0.2%</td><td class="desc-cell">씬 변경 시 MaterialBin 캐싱</td></tr>
    <tr class="highlight-row"><td class="mono-cell">Nanite::ShadeBinning</td><td class="num-cell">~7%</td><td class="num-cell">~1.3%</td><td class="num-cell excl-cell">~1.3%</td><td class="desc-cell">머티리얼 분류 Compute 레코딩</td></tr>
    <tr class="highlight-row"><td class="mono-cell">Nanite::EmitDepthTargets</td><td class="num-cell">~7%</td><td class="num-cell">~1.3%</td><td class="num-cell excl-cell">~1.2%</td><td class="desc-cell">Depth 추출 패스 레코딩</td></tr>
    <tr><td class="mono-cell">Nanite::InitContext</td><td class="num-cell">~22%</td><td class="num-cell">~0.7%</td><td class="num-cell">~0.7%</td><td class="desc-cell">버퍼 할당·View 초기화</td></tr>
    <tr><td class="mono-cell">NaniteMaterialListApply</td><td class="num-cell">~0.3%</td><td class="num-cell">~0.6%</td><td class="num-cell">~0.6%</td><td class="desc-cell">씬 변경 시 머티리얼 목록 적용</td></tr>
  </tbody>
</table>
<p style="color:var(--text3);font-size:12px;margin-top:-16px;">* 기준: Nanite::DrawGeometry Incl = 100%. Count·Incl·Excl 모두 이 값 대비 비율.</p>
...
<div class="callout callout-warn">
  ...
  <p><code>Nanite::BasePass</code> Excl ~15%가 CPU 최대 비용이다. 씬의 <strong>유니크 Nanite 머티리얼 수에 비례</strong>하므로, 머티리얼 인스턴스를 과도하게 세분화하면 GPU·CPU 모두 선형 증가한다. 씬 변경 시에는 <code>CacheNaniteMaterialBins</code>(Incl ~2%)가 추가로 발생한다.</p>
</div>

<span class="section-eyebrow">04 — 패스별 상세</span>
...
<!-- ④ RasterBinning -->
  <div class="pipe-item">
    ...
    <div class="pipe-body">
      ...
      <p>
        컬링 결과로 얻은 <code>VisibleClustersSWHW</code>를 바로 래스터라이즈하면 머티리얼 종류를 알 수 없다. <code>AddPass_Binning</code>은 3개의 Compute 패스로 <strong>RasterBin(머티리얼 ID)별 래스터라이즈 범위</strong>를 구성한다.
      </p>

      <div class="sub-section" style="margin-top:14px;">
        <h4>RasterBinCount</h4>
        <p>각 가시 클러스터에서 사용하는 머티리얼 정보를 읽어 <code>RasterBinMeta[RasterBin].BinSWCount</code>, <code>BinHWCount</code>를 증가시킨다. 클러스터 내 삼각형이 최대 3개 머티리얼을 사용하면 FastPath, 그 이상이면 Page에서 직접 머티리얼 테이블을 로드한다.</p>
        <p style="margin-top:6px;">Indirect Dispatch 인수는 클러스터 수 / 64 (WorkGroup당 64 thread, thread당 Cluster 1개).</p>
      </div>

      <div class="sub-section">
        <h4>RasterBinReserve</h4>
        <p>각 RasterBin이 <code>RasterBinData</code> 버퍼에서 차지할 오프셋을 계산한다. <code>RangeAllocator</code>(uint32 atomic counter)에서 SW+HW 클러스터 총합만큼 영역을 할당받아 <code>RasterBinMeta[RasterBin].ClusterOffset</code>에 저장한다. 또한 각 RasterBin의 Indirect DrawCall argument 초기값을 <code>RasterBinArgsSWHW</code>에 기록한다.</p>
      </div>
      ...
      <div class="sub-section">
        ...
        <p style="margin-top:6px;">완료 후 각 RasterBin은 자신이 그려야 할 클러스터 집합과 삼각형 범위를 정확히 알게 된다. 이후 SW/HW 래스터라이즈에서 <code>RasterBinArgsSWHW</code>를 Indirect Argument로 사용해 RasterBin별로 드로콜을 발행한다.</p>
      </div>
...
<!-- ⑦ HZB 생성 -->
  <div class="pipe-item">
    <div class="pipe-num">⑦</div>
    <div class="pipe-body">
      <div class="step-badge badge-cull">HZB 생성 — VisibilityBuffer → Hierarchical Z-Buffer</div>
...
<!-- ⑧ EmitDepthTargets -->
  <div class="pipe-item">
    ...
    <div class="pipe-body">
      ...
      <h3>GPU Excl ~5% · FullScreen Quad 3회</h3>
      <p>
...
<!-- ⑨ ShadeBinning -->
  <div class="pipe-item">
    ...
    <div class="pipe-body">
      ...
      <h3>GPU Excl ~4% · 3 Compute 패스로 머티리얼별 Indirect DrawArg를 준비한다</h3>
      <p>
        BasePass는 머티리얼별로 Indirect DrawCall을 1회 발행한다. 어떤 머티리얼이 화면 어느 타일(64×64 픽셀)에 존재하는지를 먼저 파악해야 한다.
      </p>

      <div class="sub-section" style="margin-top:14px;">
        <h4>InitializeMaterials</h4>
        <p>MaterialSlot 개수/64개 WorkGroup을 Dispatch. 각 MaterialSlot의 <code>MaterialIndirectArgs</code>(DrawCall argument)를 초기화하고 <code>MaterialTileRemap</code> 버퍼를 0으로 클리어한다.</p>
      </div>
      ...
      <div class="sub-section">
        ...
        <p style="margin-top:6px;">루프 완료 후 <code>MaterialTileRemap[MaterialSlot][TileLinear]</code>를 비트마스크로 기록. 각 머티리얼이 어떤 타일을 그려야 하는지 정보가 저장된다. 또한 해당 MaterialSlot의 <code>MaterialIndirectArgs.InstanceCount</code>를 타일 수만큼 증가시킨다.</p>
      </div>
...
<!-- ⑩ BasePass Material Pass -->
  <div class="pipe-item">
    <div class="pipe-num">⑩</div>
    <div class="pipe-body">
      <div class="step-badge badge-shade">BasePass — Material Pass → GBuffer</div>
      <h3>GPU Excl ~19% (최대) · CPU Excl ~15% (최대)</h3>
      <p>
        GPU·CPU 양쪽에서 모두 가장 비싼 패스다. 머티리얼 수만큼 Indirect DrawCall이 발행된다.
      </p>

      <div class="sub-section" style="margin-top:14px;">
        <h4>BuildNaniteMaterialPassCommands — CPU 드로콜 빌딩</h4>
        <p>FScene에 등록된 모든 Nanite 머티리얼(FNaniteMaterialEntry)을 순회해 <code>FNaniteMaterialPassCommand</code>를 생성한다. WPO 사용 여부로 두 개의 패스로 분리된다.</p>
        <p style="margin-top:6px;">
      ...
      <div class="sub-section">
        ...
        <p><code>SV_InstanceID</code>는 현재 머티리얼이 그릴 타일 순번(0, 1, 2...)이다. 선형 순번을 실제 타일 인덱스로 변환하기 위해 <code>MaterialTileRemap</code>에서 비트마스크를 읽어 TargetTileCount번째 bit가 1인 위치를 탐색한다.</p>
...
<!-- ⑪ Readback -->
  <div class="pipe-item">
    ...
    <div class="pipe-body">
      ...
      <h3>GPU Excl ~8% · GPU→CPU 동기 읽기</h3>
      <p>
      ...
      <p style="margin-top:8px;">Readback 패스에서 이 요청 버퍼를 CPU로 읽어와 다음 프레임의 클러스터 스트리밍을 스케줄링한다. GPU→CPU 동기 읽기이므로 <strong>파이프라인 버블</strong>을 유발할 수 있다. GPU Excl ~8%는 이 대기 시간을 포함한다.</p>
...
<div class="summary-box">
  <h3>전체 흐름 요약</h3>
  <p>
    에셋 빌드 시 메시 → <strong>Cluster 계층 (최대 128 삼각형)</strong>. 씬 등록 시 <strong>MeshDrawCommand 캐싱</strong>으로 RasterBin/ShadingBin 생성. 런타임에는 <strong>Two-Pass Occlusion Culling</strong>으로 가시 Cluster 선별 → <strong>RasterBinning</strong>으로 머티리얼별 래스터 범위 구성 → <strong>SW/HW Rasterize</strong>로 VisibilityBuffer 작성 → <strong>EmitDepthTargets</strong>으로 Depth·MaterialDepth 추출 → <strong>ShadeBinning</strong>으로 타일 분류 → <strong>BasePass</strong>에서 머티리얼별 Indirect DrawCall로 GBuffer 생성. GPU·CPU 최대 병목은 모두 <strong>유니크 머티리얼 수에 비례하는 BasePass</strong>다.
  </p>
</div>
